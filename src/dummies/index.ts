...
export const generateDummyExpenses = async () => {
  const userInfos = dummyUsers.map((user) => ({
    id: user.id,
    currency: user.currency,
  }));

  const groupInfos = groups.map((group, idx) => {
    const members = group.members as typeof dummyUsers;
    return {
      id: idx,
      type: 'household' as const,
      memberIds: members.map((m) => dummyUsers.indexOf(m)),
      defaultCurrency: group.defaultCurrency,
      createdAt: new Date('2024-01-01'),
    };
  });

  return generateAllExpenses(userInfos, groupInfos);
};
